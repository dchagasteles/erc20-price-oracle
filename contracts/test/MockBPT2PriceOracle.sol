...
////////////////////////////////////////////////////////////////////////////////////////////
/// @title MockBPT2PriceOracle
/// @author @conlot-crypto
/// @notice oracle for Balancer BPT V2
////////////////////////////////////////////////////////////////////////////////////////////

contract MockBPT2PriceOracle is IOracleV2 {
    ...
    function getUSDBalances() internal view returns (uint256[] memory usdBalances) {
        ...
        // ========== mock balances to test GeometricMean logic ============
        uint256[] memory mockBalances = balances;
        mockBalances[0] = (mockBalances[0] * 110) / 100;
        ...
        for (uint256 index = 0; index < tokens.length; index++) {
            ...
            if (18 >= decimals[index]) {
                bi = BNum.bmul(mockBalances[index], BNum.BONE * (10**(18 - decimals[index])));
            } else {
                bi = BNum.bdiv(mockBalances[index] * BNum.BONE, 10**(decimals[index] - 18));
            }
    ...
    function getComparePrices() public view returns (int256, int256) {
        uint256[] memory usdTotals = getUSDBalances();
        int256 arithmetic = int256(getArithmeticMean(usdTotals) / PriceDeviationLibrary.EXPAND);
        int256 wegihted = int256(
            PriceDeviationLibrary.getWeightedGeometricMean(usdTotals, pool.totalSupply(), params) /
                PriceDeviationLibrary.EXPAND
        );
        return (arithmetic, wegihted);
    }
}
