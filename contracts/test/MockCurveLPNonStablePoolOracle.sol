...
////////////////////////////////////////////////////////////////////////////////////////////
/// @title MockCurveLPNonStablePoolOracle
/// @author @conlot-crypto
/// @notice oracle for curve.fi LP token of NonStable Pool
////////////////////////////////////////////////////////////////////////////////////////////

contract MockCurveLPNonStablePoolOracle is IOracleV2 {
    ...
    function getUSDBalances() internal view returns (uint256[] memory usdBalances) {
        ...
        for (uint256 index = 0; index < tokens.length; index++) {
            ...
            uint256 poolBalance = pool.balances(index);
            if (index == 0) {
                poolBalance = (poolBalance * 110) / 100;
            }

            uint256 bi;
            if (18 >= decimals[index]) {
                bi = BNum.bmul(poolBalance, BNum.BONE * (10**(18 - decimals[index])));
            } else {
                bi = BNum.bdiv(poolBalance * BNum.BONE, 10**(decimals[index] - 18));
            }
    ...
    function getComparePrices() public view returns (int256, int256) {
        uint256[] memory usdTotals = getUSDBalances();
        int256 arithmetic = int256(getArithmeticMean());
